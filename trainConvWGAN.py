...
G_optim = torch.optim.RMSprop(convNet.parameters(), lr=5e-5)
D_optim = torch.optim.RMSprop(convDisc.parameters(), lr=5e-5)
...
def train_GAN_batch():
    ...
    # Train discriminator more often (Wasserstein)
    for _ in range(5):
    ...
    return errD_real.item(), errD_fake.item(), errG.item(), D_x, D_G_z1, D_G_z2
