...
def train_GAN_batch():
    ...
    of the GAN training and returns the loss for one batch.
    '''
    discriminator.train()
    transformer.train()
    ...
    for _ in range(1):
        ...
        # Calculate loss and gradients
        loss_real = loss_BCE(p_real, labels)
        loss_real.backward()
        ...
        # Calculate loss and gradients
        loss_fake = loss_BCE(p_fake, labels)
        loss_fake.backward()
    ...
    # -> The loss is minimal when D(G(z)) = real label (=1)
    labels.fill_(real_label)     
    
    # Calculate loss and new gradients
    loss_gen = loss_BCE(p_fake, labels)
    loss_gen.backward()
    
    # Update Generator (Transformer-Decoder-Part)
    optimizer.step()
    
    return loss_disc, loss_gen.item(), D_x, D_G_z1, D_G_z2
