...
class PositionalEncoding(nn.Module):
    '''
    This class adds a positional encoding to the sequence embedding 
    to introduce a notion of time dependend order.
    'influence' determines how far the position can have an effect 
    on a token.
    ...
    def __init__(self,
        ...
        pos = torch.arange(0, influence).reshape(influence, 1)
        pos_embedding = torch.zeros((influence, d_model))
        pos_embedding[:, 0::2] = torch.sin(pos * den)
        pos_embedding[:, 1::2] = torch.cos(pos * den)[:,:int(d_model/2)]
        
        # Add batch dimension (influence,d_model) -> (1,influence,d_model)
...
class TransformerModel(nn.Module):
    ...
    def encode(self,
        ...
        bz, _, _ = src_pad.shape
        src_emb = torch.zeros((bz, self.maxlen, self.d_model),device=DEVICE) 
    ...
    def decode(self,
        ...
        tgt_emb = torch.zeros((bz, seqlength, self.d_model),device=DEVICE) 
        ...
        memory_dec = self.latentDecoder(memory).reshape((bz, self.maxlen, self.d_model))
    ...
    def greedy_decode_bz(self,
                         memory: Tensor,
                         maxlen: int,
                         num_variables: int,
                         SOS: int):
        '''
        Decodes a batch of latent points into 
        a time series without using a given masked output sequence
        as during training. 
        '''
        bz = memory.shape[0]
        ys = torch.ones((bz,1,num_variables),device=DEVICE)*SOS
        
        for i in range(maxlen-1): #-1 because SOS
    ...
    def greedy_decode(self,
                      ...
                      EOS: int):
        '''
        Decodes a single point from the latent space (memory) into 
        a time series without using a given masked output sequence
        as during training. Here, varying sequence lengths are allowed
        and the algorithm stops if maxlen or the End-Of-Sequence
        EOS-Token is generated.
        '''
        #print('Start greedy_decode() ...')
        
        EOS_ = torch.ones((1,num_variables),device=DEVICE)*EOS  
        ys = torch.ones((1,1,num_variables),device=DEVICE)*SOS
        
        # ys shape: (bz = 1, len = 1, num_variables)
        for i in range(maxlen-1): #-1 because SOS, -2 if SOS+EOS          
            # Generate square subsequent mask
            tgt_mask = (torch.triu(torch.ones((ys.shape[1], ys.shape[1]), device=DEVICE)) == 1).transpose(0, 1)
            tgt_mask = tgt_mask.float().masked_fill(tgt_mask == 0, float('-inf')).masked_fill(tgt_mask == 1, float(0.0))
            tgt_mask.to(DEVICE)
            ...
            if out == EOS_:  
                break            
        ys = torch.cat([ys, torch.FloatTensor(np.array([EOS_]))], dim=1)          
        return ys
    ...
    def sample(self, bz: int = 1):
        ...
        '''
        memory = torch.rand((bz, self.d_latent),device=DEVICE)
...
class Discriminator(nn.Module):
    ...
    def forward(self,
        ...
        src_emb = torch.zeros((bz, maxlen, self.d_model),device=DEVICE) 
